"""
Targeted comparison: K=5 and K=10, uniform vs CANAL (water-filling),
at epsilon=0.5 and epsilon=1.0, on ISIC.

Per Dr. Tian's request (Jun 20-21, 2026 followup): skip K=1, K=3,
subsample, and Joint variants entirely. Only K=5 and K=10, only
uniform vs CANAL, only the two new epsilons not covered by the
existing full ISIC run (which covered eps=2,4,8,16).

Reuses the exact same functions and calling conventions as
run_comparison.py (train_K_teachers, shared_importance,
thresholded_uniform_sigma, correct_waterfilling_sigma,
precompute_joint_cache, train_student_distill) for full consistency
with the existing ISIC results.

Saves results/isic_canal_small_eps_results.json
"""
...
def main():
    ...
    ap.add_argument("--dataset", default="isic", choices=["drive", "isic", "brats"])
    ap.add_argument("--size", type=int, default=96)
    ap.add_argument("--seeds", type=int, default=5)
    ap.add_argument("--epsilons", type=str, default="0.5,1")
    ...
    # teachers for K=5,10 only (trained once, reused)
    ...
    print("\n[K=5 and K=10: uniform vs CANAL at small epsilon]")
    R["series"]["PATE K=5 uniform"]  = sweep(5, canal=False)
    R["series"]["PATE K=5 + CANAL"]  = sweep(5, canal=True)
    R["series"]["PATE K=10 uniform"] = sweep(10, canal=False)
    R["series"]["PATE K=10 + CANAL"] = sweep(10, canal=True)

    out_path = HERE / "results" / f"{args.dataset}_canal_small_eps_results.json"
