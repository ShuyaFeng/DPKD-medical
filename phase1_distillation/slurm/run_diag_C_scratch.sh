#!/bin/bash
# ============================================================
# DIAGNOSTIC C — student from random initialization
#
# Hypothesis: existing results are flat (~88.9%) because the
# student starts from a pre-trained 87.91% checkpoint and is
# already near the ceiling. Re-train from random init so the
# distillation signal actually has room to matter.
#
# Setup matches GKD_V2_uniform_eps8 except --student-from-scratch.
# Compare against:
#   GKD_V2_uniform_eps8 mean=88.77%  (from-checkpoint)
#   If from-scratch lands much lower (~85% or below) → confirms
#   the checkpoint init is masking the distillation effect.
#
# Submit:
#   for SEED in 0 1 2; do sbatch run_diag_C_scratch.sh $SEED; done
#
# NOTE: from-scratch may need more iters. Bumping to 60000 here.
# ============================================================
#SBATCH --job-name=GKD_DIAG_C
...
#SBATCH --output=/data/user/home/ialam/mmsegmentation/slurm_out/diag_C_s%1_%j.out
#SBATCH --error=/data/user/home/ialam/mmsegmentation/slurm_out/diag_C_s%1_%j.err

SEED=$1
if [ -z "$SEED" ]; then
    echo "Usage: sbatch run_diag_C_scratch.sh <seed>"
    exit 1
fi
...
export OUT=$MMSEG/work_dirs/GKD_DIAG_C_scratch_seed${SEED}
...
echo "=========================================="
echo "DIAGNOSTIC C — student from random init"
echo "Seed   : $SEED"
echo "Output : $OUT"
echo "=========================================="
