...
contains neither fresh teacher queries nor fresh noise samples. In the
experiments reported here the clipping caps---and the importance scores
entering~\eqref{eq:wf} when water-filling is used---are estimated
directly on the private training data: each teacher's caps are computed
on its own (private) cohort and used to normalize and de-normalize that
teacher's released features. These statistics are therefore
data-dependent, so the released features carry a residual dependence on
them that the per-release $(\varepsilon,\delta)$ accounting of
\S\ref{ssec:accounting} does not separately charge; we flag this as a
limitation of the reported runs rather than a property of the mechanism.
A disjoint public calibration set (e.g.\ HRF~\cite{budai2013hrf}) would
remove this data dependence, but we do not rely on one: the privacy
guarantee of the reported pipeline rests on the sample-once release and
the $\Delta=2/K$ sensitivity, not on a zero-cost public calibration.
Pseudocode is Algorithm~\ref{alg:precompute} in
